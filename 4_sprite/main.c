...
export uint* getsprite(int id) {
	if (id == -1) { 
		screen[0] = SCREEN_W;  // force screen size
		screen[1] = SCREEN_H;
		return screen; 
	}
	if (id >= 0 && id < 64) {
		sprites[id][0] = clamp( 0, sprites[id][0], 64 );  // clamp sprite sizes
		sprites[id][1] = clamp( 0, sprites[id][1], 64 );
		return sprites[id];
	}
	return NULL;
}
...
export int paint(int srcid, int dstid) {
	...
	if (src[0] == 0 || src[1] == 0 || dst[0] == 0 || dst[1] == 0)  return 0;  // size check
	// paint
...
export int paintat(int srcid, int dstid, int xpos, int ypos) {
	const uint *src = getsprite(srcid);
	uint *dst = getsprite(dstid);
	// sanity check
	if (src == NULL || dst == NULL || src == dst)  return 1;
	if (src[0] == 0 || src[1] == 0 || dst[0] == 0 || dst[1] == 0)  return 0;  // size check
	// paint
	for (int y = 0; y < src[1]; y++)
	for (int x = 0; x < src[0]; x++)
		if (x+xpos >= 0 && x+xpos < dst[0] && y+ypos >= 0 && y+ypos < dst[1])
			dst[2 + (ypos + y) * dst[0] + xpos + x] = src[2 + y * src[0] + x];
	return 0;  // OK
}